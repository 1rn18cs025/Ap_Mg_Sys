...
const Insert_Resident = () => {
  ...
  async function fetchdata(){
    const response = await fetch(`${BASE_URL}insert_resident`,{
        method: "post",
        headers: {
    ...
    const responsejson = await response.json()
    // setdata(responsejson)
    // console.log(responsejson[1][0].output)
    alert(responsejson[1][0].output)
    }
...
//   }
  ...
  return (
    <div>
        ...
        <button onClick = {fetchdata} type = "submit">Insert</button>
    </div>
...
export default Insert_Resident;
